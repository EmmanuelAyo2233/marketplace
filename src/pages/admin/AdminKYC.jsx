...
import { Shield, ShieldCheck, ShieldAlert, Check, X, Search, Clock, Eye, AlertTriangle, FileText, CheckCircle, XCircle } from 'lucide-react'
...
function AdminKYC() {
  ...
  const fetchVendors = () => {
    ...
    adminAPI.getVendors()
      .then(({ data }) => {
        // Filter to show only vendors who have submitted KYC
        const kycVendors = (data || []).filter(v => v.verificationStatus && v.verificationStatus !== 'unsubmitted')
        setVendors(kycVendors)
      })
  ...
  // Filter vendors by search query and active tab status
  const filtered = vendors.filter(v => {
    const matchesSearch = (v.storeName || v.name || v.fullName || v.email || '').toLowerCase().includes(search.toLowerCase())
    if (!matchesSearch) return false

    if (activeTab === 'pending') return v.verificationStatus === 'pending'
    if (activeTab === 'approved') return v.isVerified === true || v.verificationStatus === 'approved'
  ...
  return (
    <motion.div initial="hidden" animate="show" variants={containerV} className="space-y-6">
      ...
      <motion.div variants={itemV} className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-display font-black text-slate-900 tracking-tight">KYC Verification Requests</h1>
          <p className="text-sm text-slate-500 mt-0.5">{vendors.length} submitted verification request{vendors.length !== 1 ? 's' : ''}</p>
        </div>
      ...
      <motion.div variants={itemV} className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: 'Total Requests', value: vendors.length, color: 'bg-brand-50 text-brand-600', id: 'all' },
          { label: 'Pending Review', value: vendors.filter(v => v.verificationStatus === 'pending').length, color: 'bg-amber-50 text-amber-600', id: 'pending' },
          { label: 'Approved', value: vendors.filter(v => v.isVerified).length, color: 'bg-emerald-50 text-emerald-600', id: 'approved' },
          { label: 'Rejected', value: vendors.filter(v => v.verificationStatus === 'rejected').length, color: 'bg-red-50 text-red-600', id: 'rejected' },
        ].map(s => (
          <button key={s.label} onClick={() => setActiveTab(s.id)}
            className={`bg-white rounded-2xl border p-4 text-center transition-all ${activeTab === s.id ? 'border-brand-500 ring-2 ring-brand-500/10' : 'border-slate-100 hover:border-slate-200'}`}
          >
      ...
      {/* Tabs Menu */}
      <motion.div variants={itemV} className="flex gap-2 border-b border-slate-200 pb-px">
        {[
          ...
          <button key={t.id} onClick={() => setActiveTab(t.id)}
            className={`px-4 py-2.5 text-xs font-bold transition-all relative ${activeTab === t.id ? 'text-brand-600 border-b-2 border-brand-500' : 'text-slate-500 hover:text-slate-800'}`}
          >
            {t.label}
          </button>
        ))}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/50">
                <th className="text-left px-6 py-4 font-bold text-slate-500 text-xs uppercase tracking-wider">Store / Merchant</th>
                <th className="text-left px-4 py-4 font-bold text-slate-500 text-xs uppercase tracking-wider">KYC Email / Phone</th>
                <th className="text-center px-4 py-4 font-bold text-slate-500 text-xs uppercase tracking-wider">Category</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {filtered.length === 0 ? (
                <tr><td colSpan={5} className="text-center py-12 text-sm text-slate-400">No requests found</td></tr>
              ) : filtered.map(v => (
                <tr key={v._id} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-slate-400 truncate">{v.fullName || 'No Legal Name Setup'}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-4">
                    <div className="text-xs">
                      <p className="font-semibold text-slate-700">{v.kycEmail || v.email}</p>
                      <p className="text-slate-400 mt-0.5">{v.phoneNumber || 'N/A'}</p>
                    </div>
                  </td>
                  <td className="px-4 py-4 text-center">
                    <span className="inline-flex items-center gap-1 text-xs font-bold text-slate-600 bg-slate-100 px-2.5 py-1 rounded-lg">
                      {v.businessCategory || 'N/A'}
                    </span>
                  </td>
                  <td className="px-4 py-4 text-center">
                  ...
                  <td className="px-6 py-4 text-right">
                    <button onClick={() => setSelectedVendor(v)}
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-900 text-white rounded-lg hover:bg-slate-800 transition-colors text-xs font-bold" title="Review Details">
