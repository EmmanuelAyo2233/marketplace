...
import { Store, Shield, Home, ShoppingBag, ArrowLeft } from 'lucide-react'
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (payload.role === 'vendor') {
        payload.vendorSlug = payload.storeName?.toLowerCase().replace(/[^a-z0-9]+/g, '-')
        payload.vendorDescription = payload.storeDescription
      }
      
      const { data: res } = await authAPI.register(payload)
      dispatch(setCredentials({ user: res.user, accessToken: res.accessToken }))
      toast.success(`Welcome to TradeHub, ${res.user.name.split(' ')[0]}!`)
      ...
      <div className="flex-1 flex flex-col lg:flex-row w-full h-screen">
        ...
        <div className="lg:w-[45%] relative bg-slate-900 flex flex-col items-center justify-center p-8 lg:p-16 h-[40vh] lg:h-full lg:fixed lg:top-0 lg:left-0 overflow-hidden">
          <motion.img 
            ...
            src="https://images.unsplash.com/photo-1522776342835-26343f87b8f0?auto=format&fit=crop&q=80&w=1471" 
            alt="African Local Market" 
            className="absolute inset-0 w-full h-full object-cover pointer-events-none"
          />
          <div className="absolute inset-0 bg-gradient-to-br from-brand-900/90 via-slate-900/80 to-slate-900/30 z-0 pointer-events-none" />
        ...
        <div className="lg:w-[55%] lg:ml-[45%] flex-1 flex flex-col justify-center items-center px-4 py-12 lg:py-24 pt-[100px] lg:pt-24 z-10 bg-[#F8FAFC]">
          <motion.div 
            ...
            className="w-full max-w-[480px] bg-white rounded-[1.5rem] shadow-[0_20px_60px_rgb(0,0,0,0.06)] border border-slate-100/60 p-8 sm:p-10 relative my-auto"
          >
            ...
            <div className="mb-10 text-center sm:text-left">
              <h2 className="text-3xl font-display font-extrabold text-slate-900 tracking-tight mb-2">Create Account</h2>
              <p className="text-slate-500 font-medium">Get started with your TradeHub journey.</p>
            </div>

            <RegisterForm onSubmit={onSubmit} loading={loading} defaultRole={defaultRole} />

            <div className="mt-8 text-center sm:text-left pt-8 border-t border-slate-100">
              <p className="text-slate-600 font-medium">
                Already have an account?{' '}
                <Link to="/login" className="text-brand-600 font-bold hover:text-brand-800 transition-colors">Log In</Link>
              </p>
            </div>
